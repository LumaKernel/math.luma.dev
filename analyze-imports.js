...
const files = execSync(
  'find src -type f -name "*.ts" -o -name "*.tsx" | grep -v "\\.gen\\.ts" | grep -v "\\.gen\\.tsx"',
)
  .toString()
  .trim()
  .split("\n");
...
files.forEach((file) => {
  ...
  if (
    file.endsWith(".tsx") && !content.includes('import React from "react"') &&
...
console.log(
  `Files needing extension updates: ${results.needsExtension.length}`,
);
