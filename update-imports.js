...
const analysisResults = JSON.parse(fs.readFileSync('import-analysis.json', 'utf8'));
...
function determineExtension(importPath, currentFile) {
  ...
  const relativePath = cleanPath.startsWith('./') ? cleanPath.substring(2) : cleanPath;
...
analysisResults.needsExtension.forEach(item => {
  ...
  imports.forEach(importLine => {
    ...
    const newImportLine = importLine.replace(/from\s+(['"])([^'"]+)(['"])/, 
      (match, quote1, path, quote2) => `from ${quote1}${path}${extension}${quote2}`);
...
analysisResults.needsReactImport.forEach(file => {
  ...
  content = content.slice(0, insertPosition) + reactImport + content.slice(insertPosition);
